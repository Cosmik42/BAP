...
namespace LegoTrainProject
{
	...
	public class Sections : IEnumerable<Section>
	{
		...
		public async Task Reserve(Section nextSection, Section currentSection, Hub hub)
		{
			...
			while (!hub.AbortReserve && IsRunning && nextSection.CurrentHub != null)
			{
			...
			if (nextSection.NeedsTwoSectionReleased && nextNextSection != null)
			{
				...
				while (!hub.AbortReserve && IsRunning && nextNextSection.CurrentHub != null)
				{
				...
				nextSection.IsBeingCleared = true;
				nextNextSection.CurrentHub = hub;
			}
			...
			if (currentSection.CountForwardSections() > 1 && currentSection.Switch1Hub != null)
			{
				...
				else
				{
					...
					if (nextSection.NeedsTwoSectionReleased)
					{
						if (nextSection.CountForwardSections() > 1 && nextSection.Switch1Hub != null)
							nextSection.Switch1Hub.ActivateSwitch(nextSection.Switch1Port, (nextSection.LeftSection == nextNextSection));
					}
					...
					if (!IsRunning)
					{
						...
						hub.AbortReserve = false;
						hub.IsWaitingSection = false;
						nextSection.CurrentHub = null;
						if (nextNextSection != null)
							nextNextSection.CurrentHub = null;
						return;
					}
				}
			}
			...
			int waitingCount = 0;
			int totalWaiting = (nextSection.NeedsTwoSectionReleased) ? 20 : 10;
			while (waitingCount < totalWaiting && IsRunning && !hub.AbortReserve)
			{
				// We wait 3s to make sure the train has left
				await Task.Delay(hub.ClearingTimeInMs / totalWaiting).ConfigureAwait(false);
				waitingCount++;
			}
			...
			// Special case when we need ahead
			if (nextSection.NeedsTwoSectionReleased)
			{
				nextSection.IsBeingCleared = false;
				nextSection.CurrentHub = null;
			}

			MainBoard.WriteLine(hub.Name + " has cleared its previous section", System.Drawing.Color.Green);
		...
		internal void Start(TrainProject project)
		{
			for (int j = 0; j < sections.Count; j++)
			{
				...
				newEvent.TriggerDistanceParam = 5;

				newEvent.Action = TrainProgramEvent.ActionType.ExecuteCode;
				newEvent.EventIsTriggered += async () =>
				{
					...
					{
						MainBoard.WriteLine("Event discarded - Unknown train, nothing done");
						return;
					}
